...
FROM
	...
	*/
	...
	(
		SELECT U.NomeCompleto 
		FROM TB_USUARIO AS U 
			INNER JOIN TB_MEDICO AS M ON U.IdUsuario = M.IdUsuario 
		WHERE M.IdMedico = TB_CONSULTA.IdMedico 
	) as MÉDICO,
	(
		SELECT C.NomeFantasia
		FROM TB_Clinica AS C 
			INNER JOIN TB_MEDICO AS M ON C.IdClinica = M.IdClinica
		WHERE M.IdMedico = TB_Consulta.IdMedico
	) as 'NOME CLINICA',
...
FROM
	TB_Consulta
	--INNER JOIN TB_Medico ON TB_Consulta.IdMedico = TB_Medico.IdMedico
	--INNER JOIN TB_Clinica ON TB_Medico.IdClinica = TB_Clinica.IdClinica
	INNER JOIN
	TB_Paciente ON TB_Paciente.IdPaciente = TB_Consulta.IdPaciente
	INNER JOIN
	TB_Usuario ON TB_Usuario.IdUsuario = TB_Paciente.IdUsuario
	INNER JOIN
	TB_Prontuario ON TB_Prontuario.IdProntuario = TB_Paciente.IdProntuario
	INNER JOIN
	TB_Feedback ON TB_Feedback.IdPaciente = TB_Paciente.IdPaciente